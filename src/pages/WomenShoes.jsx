    ...
    try {
      ...
      if (
        ...
        const [femaleProducts, unisexProducts] = await Promise.all([
          productApi.getByGender("FEMALE", pageForApi, itemsPerPage),
          productApi.getByGender("UNISEX", 0, itemsPerPage),
        ]);
        products = [
          ...(femaleProducts?.data?.data.result || []),
          ...(unisexProducts?.data?.data?.result || []),
        ].slice(0, itemsPerPage);
        total =
          (femaleProducts?.data?.data?.meta?.total || 0) +
          (unisexProducts?.data?.data?.meta?.total || 0);
